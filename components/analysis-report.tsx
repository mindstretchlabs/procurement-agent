...
const STAGES: AnalysisStatus[] = ["pending", "extracting", "scoring", "summarizing", "complete"];
...
export function AnalysisReport({ analysisId }: { analysisId: string }) {
  ...
  if (!data) {
    return <p className="text-sm text-ink/60">Loading analysis…{pollError ? ` (${pollError})` : ""}</p>;
  }
  ...
  return <CompletedReport data={data} />;
}

function StageProgress({ status, fileName }: { status: AnalysisStatus; fileName: string | null }) {
...
function CompletedReport({ data }: { data: AnalysisResponse }) {
